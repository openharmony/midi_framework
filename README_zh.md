...
3. **资源回收**:
   ...
   * **异常监测**: 建立连接时，客户端会将回调对象（Stub）注册至服务端。服务端的 **MIDI 客户端会话管理** 模块通过 IPC 机制订阅该对象的[**死亡通知（Death Recipient）**](https://gitcode.com/openharmony/docs/blob/master/zh-cn/application-dev/ipc/subscribe-remote-state.md)。
     * 若客户端进程异常退出（Crash），内核会通知服务端该回调代理（Proxy）已失效。
     * 服务端感知后，立即清理该客户端占用的会话与共享内存资源。
